...
async function getDisplayedScoreSourceType(page) {
  return page.evaluate(() => {
    const valueEl = document.getElementById('scoreSourceType');
    return valueEl ? (valueEl.textContent || '').trim() : null;
  });
}

async function getClassNameBySvgElementId(page, id) {
...
function buildPathDataFromSimplifiedSignature(signature) {
  ...
  for (const command of signature) {
    ...
    if (command === 'C') {
      parts.push(`C ${x} ${y} ${x + 1} ${y + 1} ${x + 2} ${y + 2}`);
      x += 3;
      y += 3;
      continue;
    }
    ...
    throw new Error(`Unsupported path command in simplified signature: ${command}`);
  }

  return parts.join(' ');
}

function buildDoricoFontGatedClefSvgBuffer() {
  const bravuraBassClefSig = 'MCCCCCCCCCCCCCCCCMCCCCMCCCC';
  const sebastianBassClefSig = 'MCLCLCCCCCCCCCCCMCCCCMCCCC';
  const bravuraCommonTimeSig = 'MCCCCCCCCCCCCCCCCCZM';
  const sebastianCommonTimeSig = 'MCCCCCCCCCCCCCCCZM';
  const bravuraSharpSig = 'MCCCCCCCCMCLCCCCLCCLCCCLCCLCCCLCCCCCLCCCLCCCCCLCCLCLCCLCCLCCCLCCLCLCL';
  const sebastianSharpSig = 'MCLLLLCLLLLCLLLLLLCLLLLCLLLLCLLLLLLMCLLCLL';
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="140" viewBox="0 0 320 140" version="1.2" baseProfile="tiny">
      <desc>Dorico</desc>
      <g font-family="Bravura">
        <line x1="20" y1="40" x2="300" y2="40" stroke="#000" stroke-width="1" />
        <line x1="20" y1="50" x2="300" y2="50" stroke="#000" stroke-width="1" />
        <line x1="20" y1="60" x2="300" y2="60" stroke="#000" stroke-width="1" />
        <line x1="20" y1="70" x2="300" y2="70" stroke="#000" stroke-width="1" />
        <line x1="20" y1="80" x2="300" y2="80" stroke="#000" stroke-width="1" />
        <line x1="28" y1="38" x2="28" y2="82" stroke="#000" stroke-width="1" />
        <path id="dorico-active-clef" d="${buildPathDataFromSimplifiedSignature(bravuraBassClefSig)}" transform="translate(40 55) scale(0.35)" />
        <path id="dorico-active-keysig" d="${buildPathDataFromSimplifiedSignature(bravuraSharpSig)}" transform="translate(86 54) scale(0.25)" />
        <path id="dorico-active-timesig" d="${buildPathDataFromSimplifiedSignature(bravuraCommonTimeSig)}" transform="translate(118 58) scale(0.24)" />
      </g>
      <g font-family="Sebastian">
        <path id="dorico-foreign-clef" d="${buildPathDataFromSimplifiedSignature(sebastianBassClefSig)}" transform="translate(120 55) scale(0.35)" />
        <path id="dorico-foreign-keysig" d="${buildPathDataFromSimplifiedSignature(sebastianSharpSig)}" transform="translate(152 54) scale(0.25)" />
        <path id="dorico-foreign-timesig" d="${buildPathDataFromSimplifiedSignature(sebastianCommonTimeSig)}" transform="translate(186 58) scale(0.24)" />
      </g>
    </svg>
  `.trim();
...
function buildMuseScoreSemanticGatedClefSvgBuffer() {
  const bravuraBassClefSig = 'MCCCCCCCCCCCCCCCCMCCCCMCCCC';
  const bravuraCommonTimeSig = 'MCCCCCCCCCCCCCCCCCZM';
  const bravuraSharpSig = 'MCCCCCCCCMCLCCCCLCCLCCCLCCLCCCLCCCCCLCCCLCCCCCLCCLCLCCLCCLCCCLCCLCLCL';
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="140" viewBox="0 0 320 140" version="1.2" baseProfile="full">
      <desc>Generated by MuseScore Studio 4.5.2</desc>
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,40 300,40" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,50 300,50" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,60 300,60" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,70 300,70" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,80 300,80" />
      <polyline class="BarLine" fill="none" stroke="#000" stroke-width="1" points="28,38 28,82" />
      <path id="musescore-semantic-clef" class="Clef" d="${buildPathDataFromSimplifiedSignature(bravuraBassClefSig)}" transform="translate(40 55) scale(0.35)" />
      <path id="musescore-generic-clef" d="${buildPathDataFromSimplifiedSignature(bravuraBassClefSig)}" transform="translate(120 55) scale(0.35)" />
      <path id="musescore-semantic-keysig" class="KeySig" d="${buildPathDataFromSimplifiedSignature(bravuraSharpSig)}" transform="translate(86 54) scale(0.25)" />
      <path id="musescore-generic-keysig" d="${buildPathDataFromSimplifiedSignature(bravuraSharpSig)}" transform="translate(152 54) scale(0.25)" />
      <path id="musescore-semantic-timesig" class="TimeSig" d="${buildPathDataFromSimplifiedSignature(bravuraCommonTimeSig)}" transform="translate(118 58) scale(0.24)" />
      <text id="musescore-generic-timesig" x="182" y="66" font-family="Bravura" font-size="28">c</text>
    </svg>
  `.trim();
...
function buildSibeliusTextGatedClefSvgBuffer() {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="140" viewBox="0 0 320 140" version="1.2" baseProfile="tiny">
      ...
      <text id="sibelius-symbol-font-timesig" x="118" y="70" font-family="Opus Std" font-size="28">c</text>
      <text id="sibelius-text-font-timesig" x="182" y="70" font-family="Opus Text Std" font-size="28">c</text>
    </svg>
  `.trim();
...
function buildDoricoForeignNoteheadAccidentalSvgBuffer() {
  const bravuraSharpSig = 'MCCCCCCCCMCLCCCCLCCLCCCLCCLCCCLCCCCCLCCCLCCCCCLCCLCLCCLCCLCCCLCCLCLCL';
  const sebastianWholeNoteheadSig = 'MCCCCMCCCCCCCC';
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="140" viewBox="0 0 320 140" version="1.2" baseProfile="tiny">
      <desc>Dorico</desc>
      <g font-family="Bravura">
        ...
        <line x1="20" y1="80" x2="300" y2="80" stroke="#000" stroke-width="1" />
        <line x1="28" y1="38" x2="28" y2="82" stroke="#000" stroke-width="1" />
        <path id="dorico-active-sharp" d="${buildPathDataFromSimplifiedSignature(bravuraSharpSig)}" transform="translate(86 54) scale(0.25)" />
      </g>
      <g font-family="Sebastian">
        <path id="dorico-foreign-notehead" d="${buildPathDataFromSimplifiedSignature(sebastianWholeNoteheadSig)}" transform="translate(112 58) scale(0.3)" />
      </g>
    </svg>
  `.trim();
...
function buildMuseScoreGenericNoteheadAccidentalSvgBuffer() {
  const bravuraSharpSig = 'MCCCCCCCCMCLCCCCLCCLCCCLCCLCCCLCCCCCLCCCLCCCCCLCCLCLCCLCCLCCCLCCLCLCL';
  const bravuraQuarterNoteheadSig = 'MCCCC';
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="140" viewBox="0 0 320 140" version="1.2" baseProfile="full">
      <desc>Generated by MuseScore Studio 4.5.2</desc>
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,40 300,40" />
      ...
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,80 300,80" />
      <polyline class="BarLine" fill="none" stroke="#000" stroke-width="1" points="28,38 28,82" />
      <path id="musescore-semantic-sharp" class="KeySig" d="${buildPathDataFromSimplifiedSignature(bravuraSharpSig)}" transform="translate(86 54) scale(0.25)" />
      <path id="musescore-generic-notehead" d="${buildPathDataFromSimplifiedSignature(bravuraQuarterNoteheadSig)}" transform="translate(116 60) scale(5)" />
      <path id="musescore-semantic-notehead" class="Note" d="${buildPathDataFromSimplifiedSignature(bravuraQuarterNoteheadSig)}" transform="translate(240 60) scale(5)" />
    </svg>
  `.trim();
...
function buildMuseScoreInstrumentNamePathSvgBuffer() {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="360" height="180" viewBox="0 0 360 180" version="1.2" baseProfile="full">
      <desc>Generated by MuseScore Studio 4.5.2</desc>
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,40 320,40" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,50 320,50" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,60 320,60" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,70 320,70" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,80 320,80" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,110 320,110" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,120 320,120" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,130 320,130" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,140 320,140" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="40,150 320,150" />
      <polyline class="BarLine" fill="none" stroke="#000" stroke-width="1" points="42,38 42,82" />
      <polyline class="BarLine" fill="none" stroke="#000" stroke-width="1" points="42,108 42,152" />
      <path id="musescore-inst-top" class="InstrumentName" fill="#000" d="M12 52 L28 52 L28 58 L12 58 Z" />
      <path id="musescore-inst-bottom" class="InstrumentName" fill="#000" d="M10 122 L30 122 L30 128 L10 128 Z" />
      <path class="Clef" fill="#000" d="M 0 0 C 0 8 4 12 10 12 C 14 12 16 8 16 4 C 16 -2 12 -6 8 -6 C 4 -6 0 -2 0 4 C 0 14 6 20 14 20" transform="translate(56 60) scale(0.8)" />
      <path class="Clef" fill="#000" d="M 0 0 C 0 8 4 12 10 12 C 14 12 16 8 16 4 C 16 -2 12 -6 8 -6 C 4 -6 0 -2 0 4 C 0 14 6 20 14 20" transform="translate(56 130) scale(0.8)" />
    </svg>
  `.trim();
...
function buildSibeliusTextDomainNoteheadAccidentalSvgBuffer() {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="140" viewBox="0 0 320 140" version="1.2" baseProfile="tiny">
      <desc>Generated with Qt</desc>
      <g fill="none" stroke="black" stroke-width="1" fill-rule="evenodd" stroke-linecap="square" stroke-linejoin="bevel">
      ...
      <text id="sibelius-active-flat" x="86" y="68" font-family="Opus Std" font-size="30"></text>
      <text id="sibelius-text-domain-notehead" x="116" y="68" font-family="Opus Text Std" font-size="30"></text>
    </svg>
  `.trim();
...
function buildMuseScoreSemanticSixEightSvgBuffer() {
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="160" viewBox="0 0 320 160" version="1.2" baseProfile="full">
      <desc>Generated by MuseScore Studio 4.5.2</desc>
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,60 300,60" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,70 300,70" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,80 300,80" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,90 300,90" />
      <polyline class="StaffLines" fill="none" stroke="#000" stroke-width="1" points="20,100 300,100" />
      <polyline class="BarLine" fill="none" stroke="#000" stroke-width="1" points="28,58 28,102" />
      <polyline class="BarLine" fill="none" stroke="#000" stroke-width="1" points="220,58 220,102" />
      <path class="TimeSig" id="musescore-six" transform="matrix(0.96378,0,0,0.96378,56,70)" d="${sixPath}" />
      <path class="TimeSig" id="musescore-eight" transform="matrix(0.96378,0,0,0.96378,56,118)" d="${eightPath}" />
    </svg>
  `.trim();
...
function buildSibeliusRaisedSixEightSvgBuffer() {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="320" height="180" viewBox="0 0 320 180" version="1.2" baseProfile="tiny">
      <desc>Generated with Qt</desc>
      <g fill="none" stroke="black" stroke-width="1" fill-rule="evenodd" stroke-linecap="square" stroke-linejoin="bevel">
        <line x1="20" y1="86" x2="300" y2="86" stroke="#000" stroke-width="1" />
        <line x1="20" y1="96" x2="300" y2="96" stroke="#000" stroke-width="1" />
...
test('classifies imported score sources as Dorico MuseScore Sibelius or Unknown', async ({ page }) => {
  ...
  for (const [fixtureName, expectedType] of fixtures) {
    const fixturePath = path.resolve(__dirname, 'fixtures', fixtureName);
    await loadFixtureIntoScore(page, fixturePath);

    await expect.poll(async () => getDetectedScoreSourceType(page), {
      message: `waiting for ${fixtureName} to classify as ${expectedType}`,
    }).toBe(expectedType);
  }
});

test('shows the detected score source type in the sources card', async ({ page }) => {
  await page.goto('/index.html');

  await expect.poll(async () => getDisplayedScoreSourceType(page), {
    message: 'waiting for initial score source label to render',
  }).toBe('-');

  const museScoreFixture = path.resolve(__dirname, 'fixtures', 'score-source-musescore.svg');
  await loadFixtureIntoScore(page, museScoreFixture);
  await expect.poll(async () => getDisplayedScoreSourceType(page), {
    message: 'waiting for MuseScore label to appear',
  }).toBe('MuseScore');

  const sibeliusFixture = path.resolve(__dirname, 'fixtures', 'score-source-sibelius.svg');
  await loadFixtureIntoScore(page, sibeliusFixture);
  await expect.poll(async () => getDisplayedScoreSourceType(page), {
...
test('accepts Dorico clef paths from known music fonts beyond the dominant analysis font', async ({ page }) => {
  ...
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'dorico-font-gated-clefs.svg',
    mimeType: 'image/svg+xml',
    buffer: buildDoricoFontGatedClefSvgBuffer(),
  });
...
test('prefers MuseScore semantic candidates over generic path guessing', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'musescore-semantic-gated-clefs.svg',
    mimeType: 'image/svg+xml',
    buffer: buildMuseScoreSemanticGatedClefSvgBuffer(),
  });

  await expect.poll(async () => ({
    semantic: await getClassNameBySvgElementId(page, 'musescore-semantic-clef'),
    generic: await getClassNameBySvgElementId(page, 'musescore-generic-clef'),
  }), {
    message: 'waiting for MuseScore semantic clef gating to settle',
  }).toEqual({
    semantic: expect.stringContaining('highlight-clef'),
    generic: expect.not.stringContaining('highlight-clef'),
  });
});
...
test('treats MuseScore InstrumentName paths as opening sticky instrument labels', async ({ page }) => {
  ...
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'musescore-instrument-paths.svg',
    mimeType: 'image/svg+xml',
    buffer: buildMuseScoreInstrumentNamePathSvgBuffer(),
  });

  await expect.poll(async () => page.evaluate(async () => {
    const { createSvgAnalysisFeature } = await import('/scripts/features/svg-analysis.js');
    const svg = document.querySelector('#svg-sandbox svg');
    const topPath = document.getElementById('musescore-inst-top');
    const bottomPath = document.getElementById('musescore-inst-bottom');
    if (!svg) {
      return {
        topClass: topPath?.className?.baseVal || '',
        bottomClass: bottomPath?.className?.baseVal || '',
        instBlocks: [],
      };
    }
    ...
    const result = svgAnalysisFeature.buildRenderQueue(svg);
    const instBlocks = Object.values(result.globalStickyLanes || {})
      .flatMap((lane) => lane?.typeBlocks?.inst || [])
      .map((block) => ({
    ...
    message: 'waiting for MuseScore instrument-name paths to become sticky opening labels',
  }).toEqual({
    topClass: expect.stringContaining('highlight-instname'),
    bottomClass: expect.stringContaining('highlight-instname'),
    instBlocks: [
      expect.objectContaining({ stickyItemCount: 1 }),
      expect.objectContaining({ stickyItemCount: 1 }),
    ],
  });
});
...
test('accepts Sibelius clefs from symbol fonts and known music fonts while rejecting text-domain fonts', async ({ page }) => {
  ...
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'sibelius-text-gated-clefs.svg',
    mimeType: 'image/svg+xml',
    buffer: buildSibeliusTextGatedClefSvgBuffer(),
  });
...
test('restricts Dorico key-signature paths to the active music font while allowing known-font time signatures', async ({ page }) => {
  ...
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'dorico-font-gated-symbols.svg',
    mimeType: 'image/svg+xml',
    buffer: buildDoricoFontGatedClefSvgBuffer(),
  });

  await expect.poll(async () => ({
    activeKeySig: await getClassNameBySvgElementId(page, 'dorico-active-keysig'),
    foreignKeySig: await getClassNameBySvgElementId(page, 'dorico-foreign-keysig'),
...
test('keeps MuseScore semantic time-signature and key-signature candidates isolated from generic fallbacks', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'musescore-semantic-gated-symbols.svg',
    mimeType: 'image/svg+xml',
    buffer: buildMuseScoreSemanticGatedClefSvgBuffer(),
  });

  await expect.poll(async () => ({
    semanticKeySig: await getClassNameBySvgElementId(page, 'musescore-semantic-keysig'),
    genericKeySig: await getClassNameBySvgElementId(page, 'musescore-generic-keysig'),
    semanticTimeSig: await getClassNameBySvgElementId(page, 'musescore-semantic-timesig'),
    genericTimeSig: await getClassNameBySvgElementId(page, 'musescore-generic-timesig'),
  }), {
    message: 'waiting for MuseScore semantic key-signature and time-signature gating to settle',
  }).toEqual({
    semanticKeySig: expect.stringContaining('highlight-accidental'),
    genericKeySig: expect.not.stringMatching(/highlight-(?:keysig|accidental)/),
    semanticTimeSig: expect.stringContaining('highlight-timesig'),
    genericTimeSig: expect.not.stringContaining('highlight-timesig'),
  });
});

test('restricts Sibelius time-signature and accidental text analysis to music-symbol fonts before fallback', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'sibelius-text-gated-symbols.svg',
    mimeType: 'image/svg+xml',
    buffer: buildSibeliusTextGatedClefSvgBuffer(),
  });

  await expect.poll(async () => ({
    symbolFlat: await getClassNameBySvgElementId(page, 'sibelius-symbol-font-flat'),
    textFlat: await getClassNameBySvgElementId(page, 'sibelius-text-font-flat'),
    symbolTimeSig: await getClassNameBySvgElementId(page, 'sibelius-symbol-font-timesig'),
    textTimeSig: await getClassNameBySvgElementId(page, 'sibelius-text-font-timesig'),
  }), {
    message: 'waiting for Sibelius accidental and time-signature text gating to settle',
  }).toEqual({
    symbolFlat: expect.stringContaining('highlight-keysig'),
    textFlat: expect.not.stringContaining('highlight-keysig'),
    symbolTimeSig: expect.stringContaining('highlight-timesig'),
    textTimeSig: expect.not.stringContaining('highlight-timesig'),
  });
});

test('does not let Dorico foreign-font noteheads seed accidental classification', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'dorico-foreign-notehead-accidental.svg',
    mimeType: 'image/svg+xml',
    buffer: buildDoricoForeignNoteheadAccidentalSvgBuffer(),
  });

  await expect.poll(async () => getClassNameBySvgElementId(page, 'dorico-active-sharp'), {
    message: 'waiting for Dorico accidental notehead gating to settle',
  }).toContain('highlight-keysig');

  const classes = await getClassNameBySvgElementId(page, 'dorico-active-sharp');
  expect(classes).not.toContain('highlight-accidental');
});

test('does not let MuseScore generic notehead paths seed semantic key-signature reclassification', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'musescore-generic-notehead-accidental.svg',
    mimeType: 'image/svg+xml',
    buffer: buildMuseScoreGenericNoteheadAccidentalSvgBuffer(),
  });

  await expect.poll(async () => getClassNameBySvgElementId(page, 'musescore-semantic-sharp'), {
    message: 'waiting for MuseScore notehead gating to settle',
  }).toContain('highlight-keysig');

  const classes = await getClassNameBySvgElementId(page, 'musescore-semantic-sharp');
  expect(classes).not.toContain('highlight-accidental');
});

test('does not let Sibelius text-domain noteheads seed accidental classification', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'sibelius-text-domain-notehead-accidental.svg',
    mimeType: 'image/svg+xml',
    buffer: buildSibeliusTextDomainNoteheadAccidentalSvgBuffer(),
  });

  await expect.poll(async () => getClassNameBySvgElementId(page, 'sibelius-active-flat'), {
    message: 'waiting for Sibelius notehead gating to settle',
  }).toContain('highlight-keysig');

  const classes = await getClassNameBySvgElementId(page, 'sibelius-active-flat');
  expect(classes).not.toContain('highlight-accidental');
});

test('decodes MuseScore semantic 6/8 path time signatures from real export glyphs', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'musescore-semantic-six-eight.svg',
    mimeType: 'image/svg+xml',
    buffer: buildMuseScoreSemanticSixEightSvgBuffer(),
  });

  await expect.poll(async () => ({
    six: await getClassNameBySvgElementId(page, 'musescore-six'),
    eight: await getClassNameBySvgElementId(page, 'musescore-eight'),
    display: await page.locator('#timeSigDisplay').textContent(),
  }), {
    message: 'waiting for MuseScore 6/8 semantic paths to decode and pair',
  }).toEqual({
...
test('recognizes raised Sibelius stacked text time signatures near the opening barline', async ({ page }) => {
  await page.goto('/index.html');
  await preserveImportedSvgDuringSmoke(page);
  await page.setInputFiles('#svgInput', {
    name: 'sibelius-raised-six-eight.svg',
    mimeType: 'image/svg+xml',
    buffer: buildSibeliusRaisedSixEightSvgBuffer(),
  });
  ...
  await expect.poll(async () => ({
    ...
    message: 'waiting for raised Sibelius 6/8 text to classify',
  }).toEqual({
    six: expect.stringContaining('highlight-timesig'),
    eight: expect.stringContaining('highlight-timesig'),
    display: '6/8',
